...
Volta's revolutionary innovation was making Tensor Cores the \textit{dominant computational unit}. Each SM contained \textbf{8 Tensor Cores}, with 640 total in V100 (80 SMs). Each Tensor Core performs a $4 \times 4 \times 4$ matrix multiply-accumulate per clock:
\begin{equation}
D = A \times B + C
\end{equation}
...
\begin{itemize}
\item \textbf{HBM2e}: 40GB (1.6 TB/s) or 80GB (2.0 TB/s), up to 2.5x V100 capacity
...
By 2026, multi-trillion parameter models (10T+), real-time multimodal AI (vision, language, audio fusion), and extended reasoning with massive test-time compute push data centers toward physical power limits (megawatts per facility). The dominant bottleneck shifts to \textbf{energy efficiency and inference token cost}: performance-per-watt and cost-per-token become the critical metrics for sustainable AI scaling.

\subsection{Architectural Response}

Rubin, officially announced at CES 2026 \cite{nvidia2024rubin}, is NVIDIA's first architecture explicitly designed for energy-constrained extreme-scale AI. Built on TSMC 3nm process with 336 billion transistors (dual reticle design):

\textbf{6th-generation Tensor Cores with NVFP4}:
\begin{itemize}
\item 50 PFLOPS NVFP4 inference performance (5x Blackwell)
\item 35 PFLOPS NVFP4 training (3.5x Blackwell)
\item Extended FP4 precision with micro-tensor scaling
\item Enhanced FP8 capabilities
\item Hardware-accelerated adaptive compression
\item Improved confidential computing for secure multi-tenant AI
\end{itemize}

\textbf{HBM4 Memory}:
\begin{itemize}
\item 288 GB capacity per GPU (1.5x Blackwell)
\item 22 TB/s bandwidth (2.75x Blackwell's 8 TB/s)
\item Compute-to-bandwidth ratio: $\sim$2273 FLOPs/byte at NVFP4
\end{itemize}

\textbf{NVLink 6}:
\begin{itemize}
\item 3.6 TB/s bidirectional bandwidth per GPU (2x NVLink 5)
\item Electrical interconnect (not optical)
\item NVL72 configuration: 260 TB/s total scale-up bandwidth across 72 GPUs
\end{itemize}

\textbf{Rubin Platform}:
\begin{itemize}
\item Vera Arm-based CPU for orchestration
\item Next-generation SuperNICs for cluster networking
\item Advanced DPUs for data processing
\item Enhanced RAS Engine for data center reliability
\end{itemize}

\subsection{Impact and Availability}

Rubin achieves 10x lower inference token cost versus Blackwell, directly addressing the energy efficiency and deployment cost bottlenecks at extreme AI scale. Full production began Q1 2026, with products shipping H2 2026.
...
NVFP4 in Rubin (2026) at 50 PFLOPS inference performance represents the current frontier, with further reduction requiring careful validation.
...
\item Rubin: ~5\% general (336B transistors, primarily NVFP4 inference)
\end{itemize}

This trend demonstrates GPUs evolving into \textit{neural processing units} (NPUs) with minimal general compute, optimized specifically for AI workloads.
...
\textbf{3. Precision halving}: Numerical precision halves every ~3 years (FP32 → FP16 → FP8 → FP4), justified by ML workloads' noise tolerance. NVFP4 in Rubin (2026) at 50 PFLOPS represents the current frontier, with further reduction requiring careful validation.
