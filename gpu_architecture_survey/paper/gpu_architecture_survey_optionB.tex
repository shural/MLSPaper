...
\textbf{Parallel execution}: Volta introduced independent thread scheduling with per-thread program counters. While threads within a warp could always diverge, Volta's new scheduler allows the GPU to pause diverging warps and schedule other ready warps more efficiently, improving utilization on code with irregular control flow and enabling fine-grained synchronization primitives. Each SM supported up to 64 warps (2048 threads), a significant increase from prior architectures, enabling finer-grained occupancy and latency hiding.
...
FP8 (sparse) & 10 PFLOPS (2.5x H100 sparse) \\
...
\midrule
Fermi (2010) & 177 GB/s & — & $\sim$8.5 FLOPs/B \\
...
Blackwell (2024) & 8 TB/s & 2.4x & $\sim$2500 FLOPs/B \\
\bottomrule
\end{tabular}
...
\textbf{4. Memory-compute co-scaling}: Bandwidth grew $\sim$2x every 2--3 years, tracking compute growth to maintain compute-bound operation. The rising FLOPs/byte ratio (8 $\to$ 597 $\to$ 2500) reflects both precision reduction and memory system improvements. Future bottlenecks will arise if memory scaling slows relative to compute.
