...
FP64 throughput & 750 GFLOPS (1/2 FP32) \\
Memory bandwidth & 177 GB/s \\
Memory capacity & up to 6 GB GDDR5 ECC \\
L2 Cache & 768 KB \\
Compute/bandwidth & $\sim$8.5 FLOPs/byte \\
\bottomrule
\end{tabular}
...
\textbf{Remaining bottleneck}: At 177 GB/s bandwidth and 1.5 TFLOPS FP32, the arithmetic intensity was insufficient for the $O(n^3)$ matrix operations dominating neural network training. General-purpose CUDA Cores delivered too little throughput for the emerging deep learning revolution.
...
\textbf{Parallel execution}: Volta introduced independent thread scheduling, departing from lockstep warp execution. Threads within a warp can now diverge and reconverge at sub-warp granularity, enabling fine-grained synchronization primitives and more efficient handling of irregular control flow. Each SM supported up to 64 warps (2048 threads), a significant increase from prior architectures, enabling finer-grained occupancy and latency hiding.
...
\begin{itemize}
\item \textbf{INT8}: 130 TOPS—4x throughput over FP16 (65 TFLOPS)
\item \textbf{INT4}: 260 TOPS—8x throughput for extreme quantization scenarios
\item \textbf{FP16}: 65 TFLOPS maintained for high-accuracy inference
\end{itemize}
...
\textbf{Memory}: Turing used GDDR6 (320 GB/s, 16 GB) rather than HBM2, reflecting data center inference GPU economics. This lower bandwidth vs.\ V100's 900 GB/s made inference latency the practical bottleneck at small batch sizes.
...
\begin{itemize}
\item \textbf{TF32} (TensorFloat-32): FP32 range with 10-bit mantissa (FP16 precision). Automatic 10x speedup for FP32 workloads with no code changes—legacy FP32 training code instantly benefits.
...
\textbf{Multi-Instance GPU (MIG)}: A100 partitions into up to 7 isolated instances, each with dedicated SMs, memory controllers, L2 cache partitions, and bandwidth—resolving the multi-tenancy bottleneck and improving cloud utilization from $\sim$30\% to $\sim$90\%.
...
\textbf{Impact}: GPT-3 175B fit on 8$\times$A100 with model parallelism; BERT training ran 4--8x faster than V100. TF32 provided instant 10x speedup for FP32 training code. The 40 MB L2 cached large activation tensors, reducing DRAM traffic for transformer workloads. Compute-to-memory-bandwidth ratio reached $\sim$195 FLOPs/byte, sustaining compute-bound operation despite growing model sizes.

\textbf{Remaining bottleneck}: Transformer attention's $O(n^2)$ complexity consumed 40\% of training time at 2048-token context. Longer contexts (8K--32K tokens) became computationally prohibitive. FP16 numerical instability began to surface for very large models.
...
\begin{enumerate}
\item \textbf{Transformer attention}: 40--60\% of training time at sequence lengths above 2048
...
\item Transformer Engine: 9x training speedup, 30x inference speedup vs.\ A100
\end{itemize}
...
\textbf{Impact}: H100 delivers 9x training speedup and 30x inference speedup over A100. GPT-4 scale training became feasible within realistic time budgets; long-context transformers (32K+ tokens) became practical. The Transformer Engine's automatic FP8 selection eliminated manual per-layer tuning. At $\sim$597 FLOPs/byte, workloads became extremely compute-bound, justifying the investment in Tensor Core throughput over bandwidth.

\textbf{Remaining bottleneck}: Attention's $O(n^2)$ complexity persists even at FP8; FlashAttention \cite{dao2022flashattention} provides algorithmic relief but architectural support for attention sparsity remains absent. Inference at trillion-parameter scale for real-time serving at scale requires a new optimization target.
...
\textbf{Blackwell Ultra variant} adds 2x attention layer acceleration and 1.5x more AI compute FLOPS targeting reasoning models with extended test-time compute.

\textbf{NVL72 rack-scale configuration}: 72 GPUs in a single NVLink domain operating as one logical GPU with 130 TB/s aggregate bandwidth. This shifts the unit of computation from individual GPU to entire rack—NVL72 can serve trillion-parameter models with single-digit millisecond latency.

\textbf{Decompression engine}: LZ4, Snappy, and Deflate decompression in hardware accelerates data analytics pipelines without CPU involvement.
...
\begin{itemize}
\item Memory bandwidth: 8 TB/s vs.\ 3.35 TB/s = 2.7x
\item Memory capacity: 192 GB vs.\ 80 GB = 2.4x
\item Transistors: 208B vs.\ 80B = 2.6x (enabled by dual-die)
\item NVL72 inference: 30x speedup over H100 for GPT-4 scale models
\item Cost efficiency: 25x lower cost-per-token vs.\ H100
\end{itemize}
...
FP8 throughput & 10 PFLOPS (5x H100) \\
FP4 throughput & 20 PFLOPS (10x H100 FP8) \\
...
\midrule
Fermi (2010) & 177 GB/s & — & $\sim$8 FLOPs/B \\
Volta (2017) & 900 GB/s & 5.1x & $\sim$139 FLOPs/B \\
Ampere (2020) & 1.6 TB/s & 1.8x & $\sim$195 FLOPs/B \\
Hopper (2022) & 3.35 TB/s & 1.9x & $\sim$667 FLOPs/B \\
Blackwell (2024) & 8 TB/s & 2.7x & — \\
\bottomrule
\end{tabular}
\caption{Memory bandwidth growth. Compute/BW at primary ML precision.}
\end{table}
...
\textbf{4. Memory-compute co-scaling}: Bandwidth grew $\sim$2x every 2--3 years, tracking compute growth to maintain compute-bound operation. The rising FLOPs/byte ratio (8 $\to$ 667) reflects both precision reduction and memory system improvements. Future bottlenecks will arise if memory scaling slows relative to compute.
