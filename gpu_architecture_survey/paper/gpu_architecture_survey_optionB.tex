...
\textbf{Bottleneck}: Multi-trillion-parameter models (10T+), real-time multimodal AI (vision, language, audio fusion), and extended reasoning with massive test-time compute push data centers toward physical power limits (megawatts per facility). \textit{Performance-per-watt and inference token cost} become the critical metrics—training trillion-parameter models on Blackwell-era hardware requires unsustainable energy consumption and deployment costs limit scaling.

\subsection{Key Innovation: 3nm Process and 6th-Generation Tensor Cores}
...
\begin{itemize}
\item \textbf{6th-gen Tensor Cores with NVFP4}: 50 PFLOPS NVFP4 inference (5x Blackwell); 35 PFLOPS NVFP4 training (3.5x Blackwell); extended FP4 precision with micro-tensor scaling; enhanced FP8 capabilities
\item \textbf{HBM4 memory}: 288 GB capacity per GPU (1.5x Blackwell); 22 TB/s bandwidth (2.75x Blackwell's 8 TB/s); compute-to-bandwidth ratio of $\sim$2273 FLOPs/byte at NVFP4
\item \textbf{NVLink 6}: 3.6 TB/s bidirectional bandwidth per GPU (2x NVLink 5); electrical interconnect; NVL72 configuration with 260 TB/s total scale-up bandwidth
...
Transistors & 336 billion (dual reticle) \\
Inference (NVFP4) & 50 PFLOPS (5x Blackwell) \\
Training (NVFP4) & 35 PFLOPS (3.5x Blackwell) \\
Memory & HBM4, 288 GB, 22 TB/s \\
Interconnect & NVLink 6 (3.6 TB/s) \\
Platform & Vera Rubin \\
Availability & Full production Q1 2026 \\
\bottomrule
\end{tabular}
\caption{Rubin platform key specifications}
\end{table}

\textbf{Impact}: 10x lower inference token cost versus Blackwell; enables training 10T+ parameter models within realistic power budgets; real-time inference for multimodal reasoning models with million-token contexts.
...
\begin{equation}
\text{FP64} \to \text{FP32} \to \text{FP16} \to \text{INT8/TF32/BF16} \to \text{FP8} \to \text{FP4 (NVFP4)}
\end{equation}
NVFP4 in Rubin (2026) represents the current frontier at 50 PFLOPS inference performance.
...
\textbf{3. Precision halving}: Numerical precision halves every $\sim$3 years (FP32 $\to$ FP16 $\to$ FP8 $\to$ FP4), enabled by ML workloads' inherent noise tolerance, quantization-aware training, and per-layer adaptive scaling. NVFP4 in Rubin (2026) at 50 PFLOPS represents the current frontier, with further reduction requiring careful validation.
